...
const BingoGrid = ({ words }) => {
  ...
  return (
    <div className="p-4 relative h-screen overflow-hidden">
      ...
      <div
        className="grid gap-0" // Remove gaps between cells
        style={{
      ...
      >
        {gridWords.map((word, index) => (
          <div
            key={index}
            className={`flex items-center justify-center text-center font-medium aspect-square p-2
              ${
                selectedCells[index]
            ...
            style={{
              // Hand-drawn border effect: using subtle shadows for a wobbly line
              borderTop: index < gridSize ? "none" : "1px solid black",
              borderLeft: index % gridSize === 0 ? "none" : "1px solid black",
              boxShadow: '2px 2px 2px rgba(0,0,0,0.1), -2px -2px 2px rgba(0,0,0,0.1)',
            }}
          >
            <span
