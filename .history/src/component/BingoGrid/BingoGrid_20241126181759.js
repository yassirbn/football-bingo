        ...
        {gridWords.map((word, index) => (
          <div
            ...
            className={`flex items-center justify-center border-2 rounded-md text-center font-medium aspect-square p-2
              ${selectedCells[index] ? "bg-transparent border-4 border-blue-500 rounded-full" : "bg-gray-100 hover:bg-blue-200 cursor-pointer"} 
              ${word === "FREE SPACE" ? "cursor-default" : ""}`}
            onClick={() => handleCellClick(index)}
          >
            <span
              className={`break-words text-xs sm:text-sm md:text-base lg:text-lg ${word === "FREE SPACE" ? "font-bold italic" : ""}`}
            >
              {word}
            </span>
          </div>
