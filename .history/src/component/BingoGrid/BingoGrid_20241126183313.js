    ...
    <div className="p-4 relative h-screen overflow-hidden">
      ...
      >
        {gridWords.map((word, index) => (
          <div
            key={index}
            className={`flex items-center justify-center border-black border-solid border-2 rounded-md text-center font-medium aspect-square p-2
              ${selectedCells[index]
              ? "bg-blue-300 text-white relative"
              : "bg-gray-100 hover:bg-blue-200 cursor-pointer"} 
              ${word === "FREE SPACE" ? "cursor-default" : ""}`}
            onClick={() => handleCellClick(index)}
          >
            {word !== "FREE SPACE" && selectedCells[index] && (
              <div className="absolute inset-0 border-4 rounded-full border-blue-500"></div>
            )}
            <span
              className={`break-words text-xs sm:text-sm md:text-base lg:text-lg font-handwritten ${word === "FREE SPACE" ? "font-bold italic" : ""}`}
            >
              {word}
            </span>
          </div>
