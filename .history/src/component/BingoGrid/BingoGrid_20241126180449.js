    ...
    <div className="p-4 relative h-screen overflow-hidden">
      ...
      >
        {gridWords.map((word, index) => (
          <div
            key={index}
            className={`flex items-center justify-center border-[0.5px] border-black border-solid text-center font-medium aspect-square p-2
              ${
                selectedCells[index]
            ...
            style={{
              // Simulate hand-drawn lines by adding slight shadow to grid cells
              boxShadow: '1px 1px 2px rgba(0,0,0,0.1), -1px -1px 2px rgba(0,0,0,0.1)',
              borderRight:
                index % gridSize === gridSize - 1 ? "none" : "1px solid black",
              borderBottom:
                Math.floor(index / gridSize) === gridSize - 1
                  ? "none"
                  : "1px solid black",
              borderTop: "1px solid black", // Adding top border for the first row
              borderLeft: "1px solid black", // Adding left border for the first column
            }}
          >
            <span
              className={`break-words text-xs sm:text-sm md:text-base lg:text-lg font-handwritten ${
