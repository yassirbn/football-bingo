      ...
      {bingo && (
        <div className="absolute inset-0 z-20 flex justify-center items-center pointer-events-none">
          ...
          {Array.from({ length: 150 }).map((_, idx) => (
            <div
              key={idx}
              className="fireworks-particle w-3 h-3 rounded-full absolute animate-fireworks"
              style={{
                "--x": `${Math.random() * 100 - 50}vw`, // Random X direction
                "--y": `${Math.random() * 100 - 50}vh`, // Random Y direction
                "--size": `${Math.random() * 5 + 5}px`, // Random size
                "--duration": `${Math.random() * 1.5 + 1.5}s`, // Random duration
